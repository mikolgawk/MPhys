...
with open('bs.json') as f:
    for jsonobj in f:
        ...
        crystal_list.append(strDict['crystal_system'])
        point_group_list.append(strDict['point_group'])        
...
for x in range(start_index,end_index+1):
    ...
    try:
        name='mp-'+ str(x)
        BS_dir= '/home/mikolaj/MPhys_project/band_structures/'
        ...
        plt.axis('off')
        plt.margins(0,0)
        my_path='/home/mikolaj/MPhys_project/band_structures_segmentation/'
        outfile = name+'.png'
        print(outfile, branches)
        

        plt.savefig(my_path+outfile, bbox_inches='tight', pad_inches = 0)
        plt.close()
