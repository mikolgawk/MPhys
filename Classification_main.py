...
# Declare constants
block_column = 96  # target number of pixels in a column
block_row = 96  # target number of pixels in a row
flatness_criteria = 0.99  # target flatness threshold

# Declare directories
dir_list = 'sorted_files_list.txt'  # directory containing the ids of the materials taken from the MP database
dir_bands = 'C:/Users/mikig/Dropbox/bands_pictures/'  # directory containing the band structure pictures of materials
dir_net = 'net.mat'  # directory containing the trained Matlab CNN

# Declare how may of the materials you want to go through, runs over the elements of the dir_list
start_index = 0
end_index = 1
...
def predictions(net_directory, array_image_mat):
    ...
    #
    # # Save the Python list to a text file
...
def flat_pair_predictions(array_mat):
    '''
    Predict how many band structures have at least one pair of neighboring flat segments
    ...
    for i in range(len(array_mat)):
        ...
        for j in range(len(array_mat[i]) - 1):
            ...
            # Check if both elements are greater than or equal to flatness criterion
...
def visualization(array_list, images_array, start_index):
    '''
    ...
    for element in images_list:
        ...
        for i in range(num_segments):
            ...
            axes[i].set_title(f'{array_list[count][i]:.4f}'[:-2], fontsize=24)
            axes[i].axis('off')  # Turn off axis labels
            if array_list[count][i] > flatness_criteria:
                rect = plt.Rectangle((0, 0), 96, 96, edgecolor='red', linewidth=7, fill=False)
            else:
                rect = plt.Rectangle((0, 0), 96, 96, edgecolor='black', linewidth=1, fill=False)
            axes[i].add_patch(rect)
        count += 1
        # Specify where you want to download the plots
        # plt.title(f'{read_materials(dir_list)[count + start_index]}', loc='center', pad=20, fontsize=16)
        # plt.savefig(f'C:/Users/mikig/Dropbox/Classified_bands/{read_materials(dir_list)[i]}.png')
        plt.show()

def save_flat_materials(flat_row_list, flat_compound_list):
    ''''''


    ''''''
    with open('row_flat_ids_099_allranges.txt', 'a') as file_row:
        for element in flat_row_list:
            file_row.write(element)
            file_row.write('\n')

    with open('compound_flat_ids_099_allranges.txt', 'a') as file_compound:
...
def main():
    ...
    flat_row_list = flat_row_predictions(array)
    flat_compound_list, compound_scores = compound_score(array)
    # save_flat_materials(flat_row_list, flat_compound_list)
    # histogram()
    visualization(array_list, images_array, start_index)
